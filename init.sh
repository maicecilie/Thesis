#!/bin/bash
# init.sh - Python + CUDA + MPI environment for DTU HPC
# Adapted for Harvard-GF project
...
CUDA_VERSION=11.8
MPI_MODULE=openmpi         # Change if you want mpich
...
# This will install only if missing
...
for pkg in "${REQUIRED_PACKAGES[@]}"; do
    if ! python -c "import ${pkg%%=*}" &> /dev/null; then
        echo "Installing $pkg..."
        if [ "$pkg" == "mpi4py" ]; then
            # Force compile from source to link with loaded MPI
